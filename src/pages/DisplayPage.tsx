// frontend/src/pages/DisplayPage.tsx (VERSÃO FINAL CONSOLIDADA)
...
interface DisplayUiConfig {
  headerBackgroundColor?: string; mainBackgroundColor?: string; footerBackgroundColor?: string; footerTextColor?: string;
  ticketCallWidget?: { backgroundColor?: string; titleColor?: string; ticketNumberColor?: string; counterNameColor?: string; };
  lastCalledWidget?: { backgroundColor?: string; textColor?: string; };
}
...
const DisplayPage: React.FC = () => {
  ...
  // 2. Carregar Estado Inicial (Recuperação de Falha)
  ...
  // 3. Sincronizar Estado Inicial (A PEÇA QUE FALTAVA)
  useEffect(() => {
    if (displayState) {
      if (displayState.currentTicket) {
        setLastCalledTicket(displayState.currentTicket);
      }
      if (displayState.history && displayState.history.length > 0) {
        // Garantimos que o histórico não repete a senha atual se ela vier no array
  ...
  // 4. Ligar Monitorização (Heartbeat)
  ...
  useEffect(() => {
    if (displayConfig?.counters) {
      ...
      const handleTicketCalled = (data: { type: string, payload: any }) => {
        if (data.type === 'TICKET_CALLED') {
          ...
          setTicketHistory(prev => {
             // Adiciona a nova ao início e remove duplicados se existirem
      ...
      channels.forEach(channel => socket.on(channel, handleTicketCalled));
      return () => { channels.forEach(channel => socket.off(channel, handleTicketCalled)); };
    }
  }, [displayConfig]);

  if (!deviceSecret) return <p className="text-red-500 p-4">Erro: Chave secreta em falta.</p>;
  if (isLoading) return <p className="text-white p-4">A carregar configuração do display...</p>;
  if (error) return <p className="text-red-500 p-4">Erro: {(error as Error).message}</p>;
    ...
    >
      ...
      <div className={`flex-grow p-4 lg:p-8 flex ${isVertical ? 'flex-col' : 'flex-row'}`}>
        ...
        <div className={`flex flex-col ${isVertical ? 'w-full h-1/3' : 'w-1/3 h-full'}`}>
          ...
          >
            <h2 className="text-center lg:text-3xl font-bold mb-4 uppercase" style={{ color: uiConfig?.lastCalledWidget?.textColor || '#ffffff' }}>ÚLTIMAS CHAMADAS</h2>
            <ul className="space-y-4" style={{ color: uiConfig?.lastCalledWidget?.textColor || '#ffffff' }}>
              {ticketHistory.length === 0 && <li className="text-center opacity-50">Histórico vazio</li>}
              {ticketHistory.map(ticket => (
                <li key={ticket.id} className="flex justify-between text-lg lg:text-2xl opacity-70 border-b border-white/10 pb-2 last:border-0">
        ...
        <div className="container mx-auto flex flex-col md:flex-row justify-between items-center gap-2">
          <div className="flex items-center space-x-2 text-center md:text-left">
            {displayConfig?.company.address && (
              <span>{displayConfig.company.address}, {displayConfig.company.postalCode} {displayConfig.company.locality}</span>
            )}
            {/* Ocultamos detalhes extra em mobile para não partir o layout */}
            <span className="hidden md:inline">
                {displayConfig?.company.address && displayConfig?.company.phone && (
                <><span>&bull;</span><span>{displayConfig.company.phone}</span></>
                )}
                {(displayConfig?.company.address || displayConfig?.company.phone) && displayConfig?.company.email && (
                <><span>&bull;</span><span>{displayConfig.company.email}</span></>
                )}
            </span>
          </div>

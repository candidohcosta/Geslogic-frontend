...
/**
 * Mede a largura do contentor com ResizeObserver e renderiza o BarChart
 * com width/height explícitos (sem ResponsiveContainer) — elimina o warning.
 */
const SizedBarChart: React.FC<{ data: HourlyData[] }> = ({ data }) => {
  const boxRef = useRef<HTMLDivElement | null>(null);
  const [width, setWidth] = useState(0);
  const HEIGHT = 112; // altura fixa equivalente a ~h-28
      ...
      {canRender ? (
        <BarChart width={width} height={HEIGHT} data={data}>
          ...
          <Bar dataKey="count" radius={[2, 2, 0, 0]}>
            {(data || []).map((entry: HourlyData, index: number) => (
              <Cell
                key={`cell-${index}`}
                fill={entry.hour === new Date().getHours() ? '#4f46e5' : '#e2e8f0'}
              />
            ))}
          </Bar>
        </BarChart>
...
const OperatorLiveDashboard: React.FC = () => {
  ...
  const { mutate: callNext, isPending: isCalling, error: callError } = useMutation({
    ...
    onSuccess: (data) => {
      if ('ticketNumber' in data) setCurrentTicket(data);
      else alert((data as any).message);
    },
  });
  ...
  const { mutate: updateStatus, isPending: isUpdatingStatus } = useMutation({
    ...
    onSuccess: (updatedTicket) => {
      if (updatedTicket.status === TicketStatus.COMPLETED || updatedTicket.status === TicketStatus.ABSENT)
  ...
  return (
    <UtilityPageTemplate
      header={{
        icon: Activity,
        title: 'Atendimento',
        subtitle: 'Inicie, acompanhe e conclua os atendimentos do seu posto.',
        actions: (
          <div className="flex items-center gap-2">
            <Button
              variant="secondary"
              onClick={() => setIsRecallModalOpen(true)}
              disabled={isHandlingTicket}
              title={isHandlingTicket ? 'Indisponível com uma senha em atendimento' : 'Pesquisar e re-chamar senha'}
            >
            ...
            <Button
              ...
              onClick={() => endSession()}
              disabled={isHandlingTicket}
              title={isHandlingTicket ? 'Termine o atendimento atual para encerrar o dia' : 'Encerrar dia de trabalho'}
            >
              Encerrar Dia
            </Button>
          </div>
      ...
      // Sem optionsBar nesta página
      accent={{ content: false, options: false }}
    >
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch min-w-0">
          ...
          <UtilitySection
            ...
            withAccent={true}                 // mantém a linha de marca no topo
            brandClassName="border-t-brand-500" // (opcional) customiza a cor da linha
            padded={true}                     // aplica p-4/md:p-5 internos
          >
            {/* HEADER LOCAL */}
            <div className="flex flex-row items-center justify-between">
              <div>
                <h2 className="text-2xl font-bold">Atendimento em Curso</h2>
                <p className="text-sm text-muted-foreground">
                  Posto:{' '}
                  <span className="text-gray-900 font-semibold">
                    {sessionDetails?.station.name}
                  </span>{' '}
                  | Balcão:{' '}
                  <span className="text-gray-900 font-semibold">
                    {sessionDetails?.counter.name}
                  </span>
                </p>
              </div>
              <div className="text-right">
                ...
                <p className="font-mono text-indigo-600 font-bold">
                  {stats?.summary.sessionDurationMinutes || 0}m ativos
                </p>
              </div>
            </div>

            {/* CORPO */}
            <div className="space-y-8 py-6 flex-grow flex flex-col justify-center">
              <div
                className={`relative p-8 border-2 rounded-2xl text-center transition-all ${
                  isHandlingTicket
              ...
              >
                {isHandlingTicket ? (
                  ...
                  <div className="py-10 md:py-20 text-gray-300">
            ...
            {/* “FOOTER” LOCAL (equivalente ao antigo CardFooter) */}
            <div className="flex flex-wrap justify-center gap-2 border-t bg-gray-50/50 p-4 rounded-b-lg">
              <Button
                variant="secondary"
                size="sm"
                onClick={() => updateStatus(TicketStatus.IN_SERVICE)}
                disabled={!currentTicket || currentTicket.status !== TicketStatus.CALLED || isUpdatingStatus}
              >
              ...
              <Button
                ...
                onClick={() => updateStatus(TicketStatus.ABSENT)}
                disabled={!currentTicket || currentTicket.status !== TicketStatus.CALLED || isUpdatingStatus}
              >
              ...
              <Button
                ...
                onClick={() => updateStatus(TicketStatus.COMPLETED)}
                disabled={!currentTicket || currentTicket.status !== TicketStatus.IN_SERVICE || isUpdatingStatus}
              >
          ...
          <UtilitySection className="flex flex-col gap-4 min-w-0">

            {/* Contêiner interno com gap garante separação entre TODOS os blocos */}
            <div className="flex flex-col gap-4">

              <div className="bg-brand-600 text-white rounded-2xl p-5 shadow-xl flex flex-col justify-between">
              ...
              <div className="grid grid-cols-1 gap-3">
                <MiniStatCard
                  title="Atendidos"
                  value={stats?.summary.totalServed || 0}
                  icon={<CheckCircle2 className="w-4 h-4 text-green-600" />}
                  color="bg-green-50"
                />
                <MiniStatCard
                  title="Média"
                  value={formatSeconds(stats?.summary.avgServiceTime || 0)}
                  icon={<Clock className="w-4 h-4 text-blue-600" />}
                  color="bg-blue-50"
                />
                <MiniStatCard
                  title="Ausências"
                  value={stats?.summary.absentCount || 0}
              ...
              <Card className="shadow-sm flex-grow">
                ...
                <CardContent className="p-4">
                  <SizedBarChart data={stats?.hourlyDistribution || []} />
                </CardContent>
              </Card>
        ...
        {isTransferModalOpen && currentTicket && (
          <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
            <Card className="w-full max-w-lg shadow-2xl">
              <CardHeader>
                <CardTitle>Transferir Senha: {currentTicket.ticketNumber}</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid w-full gap-1.5">
                  ...
                  <Select onValueChange={setTransferTargetServiceId} value={transferTargetServiceId}>
                    ...
                    <SelectContent>
                      {servicesForTransfer.map((service: any) => (
                ...
                <div className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => setKeepPriority(Boolean(checked))}
                  />
                  <Label htmlFor="keepPriority" className="text-sm">
                    Manter Prioridade na nova fila
                  </Label>
                </div>
              </CardContent>
              <CardFooter className="justify-end space-x-2 border-t pt-4">
                ...
                <Button onClick={handleTransferClick} disabled={isTransferring || !transferTargetServiceId} className="bg-indigo-600">
                  Confirmar
                </Button>
              </CardFooter>
            </Card>
        ...
        {isRecallModalOpen && (
          <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
            <Card className="w-full max-w-lg shadow-2xl">
              <CardHeader>
                <CardTitle>Re‑chamar Senha</CardTitle>
                <CardDescription>Pesquise uma senha do dia de hoje para re‑atendimento.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <Input
                  placeholder="Pesquisar número (ex: A005)..."
                  value={searchQuery}
                ...
                <div className="space-y-2 h-48 overflow-y-auto pr-2">
                  ...
                  {searchResults.map((ticket: TicketData) => (
                    <div
                      key={ticket.id}
                      className="flex justify-between items-center p-3 border rounded-xl hover:bg-indigo-50 transition-colors group"
                    >
                      <div>
                        <p className="font-black text-indigo-900">{ticket.ticketNumber}</p>
                        <p className="text-[10px] text-gray-500 uppercase">{ticket.service.name}</p>
                      </div>
                      <Button
                        size="sm"
                        variant="outline"
                  ...
                  {!isSearching && searchResults.length === 0 && searchQuery && (
                    <p className="text-center py-8 text-gray-400 text-sm">Nenhuma senha encontrada.</p>
                  )}
                </div>
              </CardContent>
              <CardFooter className="justify-end border-t pt-4">
                <Button variant="outline" onClick={() => setIsRecallModalOpen(false)}>
                  Fechar
                </Button>
              </CardFooter>
            </Card>
...
export default OperatorLiveDashboard;